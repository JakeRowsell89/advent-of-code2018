...
const sleepValues = Object.keys(sleepRecords).map(key => {
  const {timesMinuteSlept, mostSleptMinute} = getMostSleptMinute(sleepRecords[key])
  return { timesMinuteSlept, mostSleptMinute, guardId: key }
}).reduce((p, c) => {
  console.log(c)
  if (c.timesMinuteSlept > p.timesMinuteSlept) {
    return c
  }
  return p
}, {timesMinuteSlept: 0})
console.log(sleepValues)
console.log(sleepValues.mostSleptMinute * Number(sleepValues.guardId.replace('#', '')))
...
function getMostSleptMinute(sleepRecord) {
  ...
  const amountAtMaxPos = sleepRecord.filter(x => x.charAt(maxPosition) === SLEEP_SYMBOL).length

  // return maxPosition  * amountAtMaxPos
  return { mostSleptMinute: maxPosition, timesMinuteSlept: amountAtMaxPos }
}
